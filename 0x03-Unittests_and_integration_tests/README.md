## 03 - Unittests and Integration Tests
...
#### `utils.py` (or [download](utils.py "download"))
...
#### `client.py` (or [download](client.py "download"))
...
<details>
<summary>0. Parameterize a unit test</summary>

Get to know the function `utils.access_nested_map` and its functionality. Experiment with it in the Python console to ensure your understanding.

Your task is to write the initial unit test for `utils.access_nested_map`.

Construct a `TestAccessNestedMap` class that extends `unittest.TestCase`.

Develop the `TestAccessNestedMap.test_access_nested_map` method to verify that the method produces the expected output.
...
For each input, use `assertEqual` to confirm that the function yields the anticipated result.

Ensure that the body of the test method does not exceed 2 lines.
...
Develop the method `TestAccessNestedMap.test_access_nested_map_exception`. Utilize the `assertRaises` context manager to verify that a `KeyError` is triggered for the following inputs (employ `@parameterized.expand`):
...
Also, ensure that the error message aligns with expectations.
...
<details>
<summary>2. Mock HTTP calls</summary>

Get acquainted with the `utils.get_json` function.

Create the `TestGetJson(unittest.TestCase)` class and develop the `TestGetJson.test_get_json` method to verify that `utils.get_json` produces the expected output.

We don't want to execute any real external HTTP requests. Use `unittest.mock.patch` to patch `requests.get`. Ensure it returns a Mock object with a `json` method that yields `test_payload`, which you parametrize along with the `test_url` that you will pass to `get_json` with the following inputs:
...
Confirm that the mocked `get` method was invoked exactly once (per input) with `test_url` as an argument.

Check that the result of `get_json` matches `test_payload`.
...
<details>
<summary>3. Parameterize and patch</summary>

Study memoization and get to know the `utils.memoize` decorator.

Create the `TestMemoize(unittest.TestCase)` class and include a `test_memoize` method.

Within `test_memoize`, define the following class:
...
Use `unittest.mock.patch` to mock `a_method`. Verify that when `a_property` is invoked twice, the correct result is returned but `a_method` is only called once using `assert_called_once`.
...
<details>
<summary>4. Parameterize and patch as decorators</summary>

Get to know the `client.GithubOrgClient` class.

In a fresh `test_client.py` file, define the `TestGithubOrgClient(unittest.TestCase)` class and create the `test_org` method.

This method should verify that `GithubOrgClient.org` yields the correct result.

Employ `@patch` as a decorator to ensure `get_json` is invoked once with the anticipated argument but prevent its execution.

Use `@parameterized.expand` as a decorator to parametrize the test with a few `org` examples to pass to `GithubOrgClient`, in this sequence:

- google
- abc

Naturally, no external HTTP calls should be made.
...
<details>
<summary>5. Mocking a property</summary>

The `memoize` decorator transforms methods into properties. Research how to mock a property (refer to the provided resource).

Develop the `test_public_repos_url` method to perform a unit test on `GithubOrgClient._public_repos_url`.

Use `patch` as a context manager to patch `GithubOrgClient.org` and ensure it returns a predefined payload.

Verify that the output of `_public_repos_url` matches the expected value based on the mocked payload.
...
<details>
<summary>6. More patching</summary>

Develop the `TestGithubOrgClient.test_public_repos` method to perform a unit test on `GithubOrgClient.public_repos`.

Employ `@patch` as a decorator to mock `get_json` and ensure it returns a payload of your preference.

Use `patch` as a context manager to mock `GithubOrgClient._public_repos_url` and make it return a value of your selection.

Verify that the list of repositories matches your expectations from the selected payload.

Check that both the mocked property and the mocked `get_json` were invoked once.
...
<details>
<summary>7. Parameterize</summary>

Develop the `TestGithubOrgClient.test_has_license` method to perform a unit test on `GithubOrgClient.has_license`.
...
You should also parametrize the expected return value.
...
<details>
<summary>8. Integration test: fixtures</summary>

We aim to conduct an integration test on the `GithubOrgClient.public_repos` method, meaning we will only mock code that initiates external requests.

Establish the `TestIntegrationGithubOrgClient(unittest.TestCase)` class and incorporate the `setUpClass` and `tearDownClass` methods, which are part of the `unittest.TestCase` API.

Use `@parameterized_class` to decorate the class and parameterize it with fixtures located in `fixtures.py`. The file includes the following fixtures:

```sh
org_payload, repos_payload, expected_repos, apache2_repos
```

The `setUpClass` should mock `requests.get` to return sample payloads found in the fixtures.

Employ `patch` to initiate a patcher named `get_patcher`, and use `side_effect` to ensure the mock of `requests.get(url).json()` returns the correct fixtures for the various `url` values you expect to receive.

Implement the `tearDownClass` class method to halt the patcher.
...
<details>
<summary>9. Integration tests</summary>

Develop the `test_public_repos` method to perform a test on `GithubOrgClient.public_repos`.

Ensure that the method yields the anticipated results according to the fixtures.

Create the `test_public_repos_with_license` method to test `public_repos` with the argument `license="apache-2.0"` and confirm that the result aligns with the expected value from the fixtures.
